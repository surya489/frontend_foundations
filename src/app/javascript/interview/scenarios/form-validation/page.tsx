import FormValidationDemo from "@/components/js/scenarios/FormValidationDemo";

export default function FormValidationPage() {
  return (
    <main className="space-y-10">
      <h1 className="text-3xl font-bold">
        Form Validation
      </h1>

      {/* PROBLEM */}
      <section>
        <h2 className="text-xl font-semibold">Problem</h2>
        <p className="text-gray-600 mt-2">
          Validate user input before submitting a form to ensure correct data.
        </p>
      </section>

      {/* NAIVE */}
      <section>
        <h2 className="text-xl font-semibold">Naive Approach</h2>

        <p className="text-gray-600 mt-2">
          Submit form without validation → leads to incorrect or invalid data.
        </p>
      </section>

      {/* SOLUTION */}
      <section>
        <h2 className="text-xl font-semibold">Optimized Approach</h2>

        <pre className="bg-gray-100 p-3 rounded text-sm">
{`if (!name) → error
if (!email.includes('@')) → error
if (password < 6) → error`}
        </pre>
      </section>

      {/* DEMO */}
      <section>
        <h2 className="text-xl font-semibold">Interactive Demo</h2>
        <FormValidationDemo />
      </section>

      {/* EXPLANATION */}
      <section>
        <h2 className="text-xl font-semibold">Explanation</h2>
        <ul className="list-disc ml-6 mt-2 text-gray-600 space-y-1">
          <li>Validate fields before submission</li>
          <li>Store errors in state</li>
          <li>Show error messages dynamically</li>
        </ul>
      </section>

      {/* REAL WORLD */}
      <section>
        <h2 className="text-xl font-semibold">Real-World Use</h2>
        <ul className="list-disc ml-6 mt-2 text-gray-600">
          <li>Signup forms</li>
          <li>Login systems</li>
          <li>Checkout forms</li>
        </ul>
      </section>

      {/* INTERVIEW */}
      <section className="border-l-4 border-black pl-4">
        <h2 className="text-lg font-semibold">Explanation</h2>
        <p className="text-gray-600 text-sm mt-2">
          Form validation ensures data correctness before submission.
          It improves user experience and prevents invalid data from reaching the backend.
        </p>
      </section>
    </main>
  );
}